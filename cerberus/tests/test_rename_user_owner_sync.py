...
def _request(tmp_path, session_manager=None, token="t", research_handler=None):
    ...
    return SimpleNamespace(
        cookies={"cerberus_session": token},
...
def test_rename_research_respects_custom_data_dir(monkeypatch, tmp_path):
    """DEEP_RESEARCH_DIR (which honours CERBERUS_DATA_DIR) is used, not a
    hardcoded relative path. Before the fix, setting CERBERUS_DATA_DIR made
