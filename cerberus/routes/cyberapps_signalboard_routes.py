...
Data stored as JSON files under data/cyberapps/signalboard/<user>/.
All routes scoped to the authenticated user via get_current_user.
"""
...
def setup_cyberapps_signalboard_routes() -> APIRouter:
    ...
    def list_sources(request: Request) -> Dict[str, Any]:
        u = _user(request)
        return {"sources": _load_json(_user_dir(u) / "sources.json", [])}
    ...
    def create_source(body: SourceCreate, request: Request) -> Dict[str, Any]:
        ...
        entry = {
            "id": src_id, "name": body.name, "url": body.url,
            "type": body.type, "category": body.category,
            "enabled": body.enabled, "color": body.color,
            "pollIntervalMinutes": body.pollIntervalMinutes,
            "itemCount": 0, "errorCount": 0, "successCount": 0, "attemptCount": 0,
        }
    ...
    def update_source(source_id: str, body: SourceUpdate, request: Request) -> Dict[str, Any]:
        ...
        sources[idx].update(body.model_dump(exclude_none=True))
        _save_json(path, sources)
    ...
    def update_source_stats(source_id: str, body: Dict[str, Any], request: Request) -> Dict[str, Any]:
        ...
        if idx is None:
            raise HTTPException(status_code=404, detail="Source not found")
        allowed = {"itemCount", "errorCount", "successCount", "attemptCount",
                   "consecutiveFailures", "lastFetchAt", "lastSuccess", "error", "dailyVolume"}
    ...
    def list_items(request: Request) -> Dict[str, Any]:
        u = _user(request)
        return {"items": _load_json(_user_dir(u) / "items.json", [])}
    ...
    def patch_item(item_id: str, body: ItemPatch, request: Request) -> Dict[str, Any]:
        ...
        items[idx].update(body.model_dump(exclude_none=True))
    ...
    def get_bookmarks(request: Request) -> Dict[str, Any]:
        u = _user(request)
        return _load_json(_user_dir(u) / "bookmarks.json", {"ids": [], "tags": {}})
    ...
    def patch_settings(body: SettingsPatch, request: Request) -> Dict[str, Any]:
        ...
        return {"ok": True, "settings": {k: v for k, v in current.items() if k != "claudeApiKey"}}
    ...
    def get_alert_rules(request: Request) -> Dict[str, Any]:
        u = _user(request)
        return {"rules": _load_json(_user_dir(u) / "alert_rules.json", [])}

    @router.put("/alert-rules")
    def save_alert_rules(rules: List[AlertRuleCreate], request: Request) -> Dict[str, Any]:
        u = _user(request)
        data = [{"id": r.id or f"rule-{uuid.uuid4().hex[:8]}",
                 "regex": r.regex, "label": r.label,
                 "severity": r.severity, "color": r.color} for r in rules]
    ...
    def get_context(request: Request) -> Dict[str, Any]:
        u = _user(request)
        return {"context": _load_json(_user_dir(u) / "context.json", {})}
