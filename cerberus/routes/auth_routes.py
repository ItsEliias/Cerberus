...
SESSION_COOKIE = "cerberus_session"
...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/cerberus`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/cerberus/cerberus` (which ntfy 404s on). ntfy itself
            ...
            headers = {
                "Title": "Cerberus connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from Cerberus. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
