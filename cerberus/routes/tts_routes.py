# routes/tts_routes.py
"""
TTS API routes — multi-provider (local Kokoro, API endpoint, browser).
"""

from typing import Optional
from fastapi import APIRouter, HTTPException
from fastapi.responses import Response
from pydantic import BaseModel
import logging

logger = logging.getLogger(__name__)


class TTSRequest(BaseModel):
    text: str
    format: str = "audio"          # "audio" or "base64"
    voice: Optional[str] = None    # per-call voice override (Kokoro voice id)


def setup_tts_routes(tts_service):
    router = APIRouter(prefix="/api/tts", tags=["tts"])

    @router.get("/voices")
    async def list_voices():
        """List available Kokoro TTS voices."""
        try:
            return {"voices": tts_service.list_voices()}
        except Exception as e:
            logger.error(f"Failed to list voices: {e}")
            raise HTTPException(status_code=500, detail=str(e))

    @router.get("/stats")
    async def get_tts_stats():
        try:
            return tts_service.get_stats()
        except Exception as e:
            logger.error(f"Failed to get TTS stats: {e}")
            raise HTTPException(status_code=500, detail=str(e))

    @router.post("/synthesize")
    async def synthesize_speech(request: TTSRequest):
        try:
            if not tts_service.available:
                raise HTTPException(
                    status_code=503,
                    detail={"message": "TTS service not available"},
                )

            voice = request.voice or None

            if request.format == "base64":
                audio_b64 = tts_service.synthesize_to_base64(request.text, voice=voice)
                if not audio_b64:
                    raise HTTPException(
                        status_code=500,
                        detail={"message": "Synthesis failed"},
                    )
                return {"audio": audio_b64, "audio_base64": audio_b64}

            audio_data = tts_service.synthesize(request.text, voice=voice)
            if not audio_data:
                raise HTTPException(
                    status_code=500,
                    detail={"message": "Synthesis failed"},
                )

            is_mp3 = audio_data[:3] == b'ID3' or (
                len(audio_data) >= 2
                and audio_data[0] == 0xff
                and (audio_data[1] & 0xe0) == 0xe0
            )
            mime = "audio/mpeg" if is_mp3 else "audio/wav"
            return Response(
                content=audio_data,
                media_type=mime,
                headers={
                    "Content-Disposition": (
                        "inline; filename=speech.mp3"
                        if "mpeg" in mime
                        else "inline; filename=speech.wav"
                    )
                },
            )

        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Synthesis error: {e}", exc_info=True)
            raise HTTPException(
                status_code=500,
                detail={"message": f"Synthesis failed: {str(e)}"},
            )

    @router.post("/clear-cache")
    async def clear_tts_cache():
        try:
            tts_service.clear_cache()
            return {"success": True, "message": "Cache cleared"}
        except Exception as e:
            logger.error(f"Failed to clear cache: {e}")
            raise HTTPException(status_code=500, detail=str(e))

    return router
