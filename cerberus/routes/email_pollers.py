                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Cerberus (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Cerberus" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "cerberus_kind" if "cerberus_kind" in cols else "'scheduled' AS cerberus_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                cerberus_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Cerberus-Origin"] = "cerberus-ui"
                outer["X-Cerberus-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", cerberus_kind or "scheduled")[:64]
                outer["X-Cerberus-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `cerberus-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `CERBERUS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `cerberus-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("CERBERUS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `CERBERUS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (CERBERUS_INPROCESS_POLLERS=0); "
            "drive `cerberus-mail poll-scheduled` externally."
        )
        return
    import asyncio
