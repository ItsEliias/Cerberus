...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the Cerberus container by design. The image ships no "
...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside Cerberus."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside Cerberus.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "cerberus-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'CERBERUS_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$CERBERUS_USER_SHELL" ] && [ -x "$CERBERUS_USER_SHELL" ]; then\n'
        f'  CERBERUS_USER_PATH="$("$CERBERUS_USER_SHELL" -ic \'printf "__CERBERUS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__CERBERUS_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$CERBERUS_USER_PATH" ]; then export PATH="$CERBERUS_USER_PATH:$PATH"; fi\n'
        f"fi\n"
        f"{shlex.quote(cmd)} 2>&1 | tee {shlex.quote(str(log_path))}\n"
        f"EC=${{PIPESTATUS[0]}}\n"
        f"echo ':::EXIT_CODE:::'$EC >> {shlex.quote(str(log_path))}\n"
