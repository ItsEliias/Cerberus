...
let _sortMode = Storage.get('cerberus-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Cerberus Chat';
...
const FOLDER_STATE_KEY = 'cerberus-folder-state';
const FOLDER_ORDER_KEY = 'cerberus-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Cerberus kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Cerberus Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('cerberus-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('cerberus-session-sort', mode);
  else Storage.remove('cerberus-session-sort');
  renderSessionList();
}
