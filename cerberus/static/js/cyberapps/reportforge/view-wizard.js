...
function _tplGrid(templates, selectedId) {
  return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;">
    ${templates.map(t => `<div class="rf-tpl-card" data-id="${t.id}" style="padding:12px;border-radius:7px;cursor:pointer;border:1px solid ${selectedId === t.id ? 'var(--red,#c0392b)' : 'var(--border,#2a2a3a)'};background:${selectedId === t.id ? 'rgba(192,57,43,.10)' : 'var(--panel,#16181e)'};transition:border-color .15s,background .15s;">
      <div style="font-size:12px;font-weight:700;color:var(--fg,#c5c9d0);">${escHtml(t.name)}</div>
      <div style="font-size:10px;color:var(--text-dim,#666);margin-top:4px;line-height:1.4;">${escHtml(t.description)}</div>
    </div>`).join('')}
  </div>`;
}

function _field(id, label, type, placeholder, required = false) {
  return `<div><label for="${id}" style="font-size:11px;color:var(--text-dim,#666);display:block;margin-bottom:6px;">${label}${required ? ' <span style="color:var(--red,#c0392b)">*</span>' : ''}</label><input id="${id}" type="${type}" placeholder="${escHtml(placeholder)}" style="width:100%;padding:7px 10px;border-radius:6px;border:1px solid var(--border,#2a2a3a);background:var(--panel,#16181e);color:var(--fg,#c5c9d0);font-size:13px;box-sizing:border-box;"${type === 'date' ? ` value="${placeholder}"` : ''}></div>`;
}
...
export function renderWizardView(container, { builtinTemplates, customTemplates, onComplete, onCancel }) {
  ...
  function _renderStep0() {
    container.innerHTML = `
      <div style="height:100%;display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:16px 20px 12px;border-bottom:1px solid var(--border,#2a2a3a);flex-shrink:0;display:flex;align-items:center;justify-content:space-between;">
        ...
        <div style="flex:1;overflow-y:auto;padding:16px 20px;"><div id="rf-tpl-grid">${_tplGrid(allTemplates, selectedTemplateId)}</div></div>
    ...
    container.querySelector('#rf-wiz-next-0').addEventListener('click', () => { step = 1; _renderStep1(); });
    container.querySelectorAll('.rf-tpl-card').forEach(card => {
      card.addEventListener('click', () => { selectedTemplateId = card.dataset.id; _renderStep0(); });
    });
  }
  ...
  function _renderStep1() {
      ...
      <div style="height:100%;display:flex;flex-direction:column;overflow:hidden;">
        ...
        <div style="flex:1;overflow-y:auto;padding:16px 20px;">
          <div style="display:flex;flex-direction:column;gap:14px;max-width:480px;">
            ...
            <div><label style="font-size:11px;color:var(--text-dim,#666);display:block;margin-bottom:6px;">Platform</label>
    ...
    const gv = id => container.querySelector(`#${id}`)?.value ?? '';
    container.querySelector('#rf-wiz-cancel2').addEventListener('click', onCancel);
    container.querySelector('#rf-wiz-back-1').addEventListener('click', () => { step = 0; _renderStep0(); });
    container.querySelector('#rf-wiz-create').addEventListener('click', () => {
      ...
      onComplete(makeBlankReport({
        id: makeId(), title: gv('rf-wiz-title') || 'Untitled Report',
        targetName: gv('rf-wiz-target'), targetIP: gv('rf-wiz-ip'),
        platform: gv('rf-wiz-platform'), assessmentDate: gv('rf-wiz-date'),
        operator: gv('rf-wiz-operator'), sections,
      }));
    });
  }
