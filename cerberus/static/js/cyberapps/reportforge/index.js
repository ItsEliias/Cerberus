 ...
 * Pentest report generator: library, wizard, editor, findings, export.
...
(function injectCSS() {
  ...
  style.textContent = `
    #rf-editor-shell textarea:focus, #rf-editor-shell input:focus, #rf-editor-shell select:focus {
      outline: 1px solid var(--red, #c0392b); border-color: var(--red, #c0392b) !important; }
    .rf-card:hover { border-color: var(--red,#c0392b) !important; }
    .rf-btn-ghost { background:transparent; border:1px solid var(--border,#2a2a3a); color:var(--fg,#c5c9d0); border-radius:5px; cursor:pointer; transition:border-color .12s,color .12s; }
...
async function _bootstrap() {
  ...
  } catch (err) { console.warn('ReportForge: failed to load reports', err); }
  try {
...
async function _saveReport(report) {
  try {
    const exists = State.get('reports').some(r => r.id === report.id);
    const data = exists
      ? await api(`${BASE}/reports/${report.id}`, 'PUT', report)
      : await api(`${BASE}/reports`, 'POST', report);
    State.upsertReport(data.report);
    State.setDirty(false);
    return true;
  } catch (err) { console.error('ReportForge: save failed', err); return false; }
}

async function _deleteReport(id) {
  try { await api(`${BASE}/reports/${id}`, 'DELETE'); State.removeReport(id); return true; } catch { return false; }
}

async function _duplicateReport(id) {
  try { const data = await api(`${BASE}/reports/${id}/duplicate`, 'POST'); State.upsertReport(data.report); return data.report; } catch { return null; }
}
...
function _render() {
  ...
  if (_editorComp && view !== 'editor') { _editorComp.destroy?.(); _editorComp = null; }
  ...
  if (view === 'library') {
    renderLibraryView(_container, {
      ...
      onDelete: async id => { if (!confirm('Delete this report?')) return; await _deleteReport(id); _render(); },
      onDuplicate: async id => { await _duplicateReport(id); _render(); },
    });
    return;
  }
  ...
  if (view === 'wizard') {
    renderWizardView(_container, {
      ...
      onComplete: async report => {
        ...
        if (ok) { State.setActiveReport(report); State.setView('editor'); _render(); }
      },
...
function init(container, _ctx) {
  ...
  State.setView('library');
  _render();
  if (!_bootstrapped) { _bootstrapped = true; _bootstrap().then(() => _render()); }
}
