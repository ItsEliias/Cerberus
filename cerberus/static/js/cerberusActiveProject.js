...
const STORAGE_KEY = 'cerberus-active-project-id';
...
function _renderBanner() {
  const bar = _el('cerberus-project-context-bar');
  const label = _el('cerberus-project-context-label');
  if (!bar) return;
...
export async function openAssistantWithProject(projectId, projectName) {
  ...
  try {
    await fetch(`/api/cerberus/projects/${projectId}/activity`, {
      method: 'POST',
...
export function initAtlasActiveProject(deps = {}) {
  ...
  _el('cerberus-project-context-clear')?.addEventListener('click', clearActiveProject);
  _renderBanner();
}

const cerberusActiveProject = {
  getActiveProjectId,
...
export default cerberusActiveProject;
