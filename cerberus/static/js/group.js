...
const GROUP_STATE_KEY = 'cerberus-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('cerberus-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('cerberus-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
