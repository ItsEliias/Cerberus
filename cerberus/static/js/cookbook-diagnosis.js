  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume Cerberus owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## Cerberus Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
