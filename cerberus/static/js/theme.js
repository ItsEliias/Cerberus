...
const LS_KEY = 'cerberus-theme';
const CUSTOM_THEMES_KEY = 'cerberus-custom-themes';
...
function deriveSyntaxColors(colors) {
  ...
  const [redH, redS, redL] = hexToHSL(colors.red || '#c0392b');
  const isDark = bgL < 50;
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'Cerberus Logo',    group: 'Sidebar' },
...
export function applyColors(colors) {
  ...
  _updateFavicon(colors.red || '#c0392b');
}
...
function _updateFavicon(fg) {
  ...
  if (routeShape) {
    ...
    svg = `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><path d='M16 2L4 7V18C4 24 10 29 16 31C22 29 28 24 28 18V7Z' fill='none' stroke='${fg}' stroke-width='2' stroke-linejoin='round'/><path d='M9 18C9 15.5 10.5 13.5 12 12C12 10 11 8.5 11 8.5C11 8.5 13 9 14 11C14.6 10.5 15.3 10.2 16 10.2C16.7 10.2 17.4 10.5 18 11C19 9 21 8.5 21 8.5C21 8.5 20 10 20 12C21.5 13.5 23 15.5 23 18' stroke='${fg}' stroke-width='1.6' fill='none' stroke-linecap='round'/></svg>`;
  }
...
export function initThemeUI() {
  ...
  allGrids.forEach(g => {
    g.querySelectorAll('.theme-swatch').forEach(sw => {
      sw.addEventListener('click', (e) => {
        ...
        if (ds) ds.value = d;
        if (ps) ps.value = p;
        if (ecs) ecs.value = ec || colors.fg || '#c5c9d0';
  ...
  document.querySelectorAll('.color-reset-btn[data-reset-effect]').forEach(btn => {
    ...
    newBtn.addEventListener('click', () => {
      const ec = document.getElementById('theme-bg-effect-color');
      if (ec) {
        const fg = currentColors.fg || '#c5c9d0';
        ec.value = fg;
        applyBgEffectColor('');
  ...
  if (effectColorPicker) {
    effectColorPicker.value = _initEffectColor || currentColors.fg || '#c5c9d0';
  ...
  if (harmonyAccentEl && _harmonyHex) {
    _harmonyHex.textContent = harmonyAccentEl.value || '#c0392b';
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'cerberus_' + (obj.name || 'theme') + '.json';
      a.click();
...
function _initPerlinFlow() {
  ...
  const _onResize = () => resize();
  window.addEventListener('resize', _onResize);
  function getColor() { const s = getComputedStyle(document.documentElement); return s.getPropertyValue('--bg-effect-color').trim() || s.getPropertyValue('--fg').trim() || '#c5c9d0'; }
  function getBg() { return getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#1a1d23'; }
