...
const _ACTION_RECENTS_KEY = 'cerberus-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'cerberus-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Cerberus';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Cerberus' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
