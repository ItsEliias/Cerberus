// ============================================
// Cerberus UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__cerberusDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Cerberus Chat';
  ...
  if (_opener) window._cerberusRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('cerberus-model-sort', mode);
  ...
  (function initHudChips() {
    ...
    // AUTH chip — derive from /api/auth/status
    fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    fetch(`${API_BASE}/api/diagnostics/services`, { credentials: 'same-origin' })
      ...
      .catch(() => {
        // Non-admin users can't read service details; treat as online since page loaded
        setChip(chipOnline, 'jx2-hud-chip-dot--online', 'ONLINE');
      });
  })();
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'cerberus-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Cerberus...');
      }
  ...
  const UI_VIS_KEY = 'cerberus-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'cerberus-toolbar-visibility';
    try {
...
// ============================================
function startCerberusApp() {
  if (window.__cerberusAppStarted) return;
  window.__cerberusAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('cerberus-doc-open-' + _curSession) === '1') {
  ...
  window._cerberusBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startCerberusApp, { once: true });
} else {
  startCerberusApp();
}
