"""Configurable app registry — load apps.json, resolve paths, launch safely."""
...
def _user_registry_path() -> Optional[Path]:
    env = (os.environ.get("CERBERUS_LAUNCHER_APPS_PATH") or "").strip()
    if env:
        return Path(env)
...
def resolve_alias(text: str) -> Optional[str]:
    ...
    if norm in index:
        return index[norm]
    # Longest-prefix match for multi-word tails (e.g. "rocket league")
    matches = [(alias, app_id) for alias, app_id in index.items() if norm == alias or norm.startswith(alias + " ")]
    if matches:
...
def resolve_app_entry(app: Dict[str, Any]) -> Dict[str, Any]:
    """Resolve one registry entry to launch target."""
    app_id = (app.get("id") or "").lower()
...
def pick_default_browser() -> Dict[str, Any]:
    for app_id in ("brave", "chrome"):
        res = resolve_app(app_id)
        if res.get("ok"):
            res["browser_id"] = app_id
            return res
    return {"ok": False, "message": "No browser found. Install Brave or Chrome, or set CERBERUS_BRAVE_PATH."}
