"""Cerberus Desktop Bridge — Windows host service for safe desktop actions."""
...
LOG_PATH = Path(os.environ.get("CERBERUS_BRIDGE_LOG", r"C:\CerberusWorkspace\Logs\desktop_bridge.log"))
DEFAULT_TOKEN = os.environ.get("CERBERUS_BRIDGE_TOKEN", "")
DEFAULT_HOST_WORKSPACE = os.environ.get("CERBERUS_BRIDGE_WORKSPACE", r"C:\CerberusWorkspace")
PORT = int(os.environ.get("CERBERUS_BRIDGE_PORT", "8765"))

logger = logging.getLogger("atlas-desktop-bridge")
logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")

app = FastAPI(title="Cerberus Desktop Bridge", version="1.2")
...
def _allowed_path(path: str) -> bool:
    ...
    try:
        ...
        return resolved.lower() == allowed_res.lower() or resolved.lower().startswith(allowed_res.lower() + "\\")
...
def run_command(
    body: CommandRequest,
    x_atlas_bridge_token: Optional[str] = Header(None, alias="X-Cerberus-Bridge-Token"),
):
    token = (x_atlas_bridge_token or "").strip()
    ...
    if cmd == "open_project_in_cursor":
        path = (args.get("path") or "").strip()
        projects_root = _norm_win(DEFAULT_HOST_WORKSPACE.rstrip("\\/") + "\\Projects")
        if not path or not _allowed_path(path):
...
def main():
    logger.info("Starting Cerberus Desktop Bridge on 127.0.0.1:%s", PORT)
