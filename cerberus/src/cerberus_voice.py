...
Configure via environment:
  CERBERUS_WHISPER_MODEL       (default: tiny.en)
  CERBERUS_WHISPER_DEVICE      (default: cpu)
  CERBERUS_WHISPER_COMPUTE_TYPE (default: int8)
"""
...
def _env_model() -> str:
    return (os.environ.get("CERBERUS_WHISPER_MODEL") or "tiny.en").strip() or "tiny.en"


def _env_device() -> str:
    return (os.environ.get("CERBERUS_WHISPER_DEVICE") or "cpu").strip() or "cpu"


def _env_compute_type() -> str:
    return (os.environ.get("CERBERUS_WHISPER_COMPUTE_TYPE") or "int8").strip() or "int8"
...
def _get_model():
    ...
    try:
        ...
        _whisper_config = cfg
        logger.info(
            "[cerberus-voice] loaded whisper model=%s device=%s compute=%s",
            model_name,
            device,
            compute_type,
        )
        ...
        logger.error("[cerberus-voice] failed to load whisper model: %s", exc, exc_info=True)
        _whisper_model = None
...
def transcribe_audio(path: str | Path, language: str = "en") -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error("[cerberus-voice] transcription failed: %s", exc, exc_info=True)
        return {
            "ok": False,
