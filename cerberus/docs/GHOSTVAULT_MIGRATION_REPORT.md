...
The Cerberus version faithfully migrates all user-facing features:

| Source Feature | Cerberus Implementation |
|---------------|------------------------|
| Vault directory (plain .md files) | SQLite-free JSON store, Fernet-encrypted |
...
| Floating capture window | New note created via API (no second window in web context) |
| AI engine (local, offline) | Omitted in v1 — UI layer only; Cerberus can route to Ollama in a future enhancement |
