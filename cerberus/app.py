...
# LOCALHOST_BYPASS permanently removed — Cerberus is a guardian build.
# Auth is always enforced. There is no legitimate production use case
# for bypassing authentication on loopback requests.
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Cerberus's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-Cerberus-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-Cerberus-Owner") or "").strip()
                ...
                pass
            # LOCALHOST_BYPASS permanently removed — auth is always enforced.
...
# Voice sessions — read-only listing over voice-tagged notes.
...
from routes.cerberus_agent_thread_routes import setup_agent_thread_routes, setup_gateway_approval_routes
...
from routes.trader_paper_routes import setup_trader_paper_routes
app.include_router(setup_trader_paper_routes())
...
async def _startup_event():
    ...
    # `CERBERUS_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("CERBERUS_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (CERBERUS_INPROCESS_TASKS=0); "
