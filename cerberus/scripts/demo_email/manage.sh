...
# a switchable 'Demo' account in Cerberus + fake seed mail.
#
...
DOCKER_DIR="${CERBERUS_DEMO_MAIL_DIR:-$HOME/docker/snappymail}"
USERS_FILE="$DOCKER_DIR/dovecot/conf/users"
DEMO_USER="demo@cerberus.local"
DEMO_PASS="demodemo"
...
case "${1:-}" in
  setup)
    ...
    echo
    echo "Done. In Cerberus, switch to the 'Demo' account to show off the inbox."
    ;;
  reseed)
    "$PY" "$HERE/seed_demo_emails.py" --reset
    ;;
