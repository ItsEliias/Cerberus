...
# The Cerberus app never calls this script automatically.
#
# USAGE
...
After a successful setup, start Cerberus:
...
_enable_nvidia_overlay() {
    ...
    if echo "${_current_cf}" | grep -qF "${_overlay_fragment}"; then
        ...
        echo
        _info "Start or restart Cerberus to apply:"
    ...
    echo
    _info "Start or restart Cerberus with the NVIDIA overlay:"
    _info "  docker compose up -d --build"
    echo
...
_mode_check() {
    echo "=== Cerberus Docker GPU diagnostic ==="
    echo
    _check_nvidia_smi
    ...
    if [ "${OPT_ENABLE_OVERLAY}" -eq 1 ]; then
        if [ "${_GPU_PASSTHROUGH_OK}" -eq 0 ]; then
            ...
            # Writing COMPOSE_FILE before passthrough works causes Cerberus to fail
